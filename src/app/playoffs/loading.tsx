    ...
    <div className="mx-auto max-w-6xl space-y-8">
      ...
      <div className="flex gap-2">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="h-10 w-28 animate-pulse bg-input" />
        ))}
      </div>
          ...
          {[4, 2, 1, 2].map((rows, col) => (
            <div key={col} className="flex flex-col gap-6">
              {Array.from({ length: rows }).map((_, row) => (
                <div key={row} className="w-36 overflow-hidden border border-rule sm:w-44">
                  <div className="flex items-center gap-2 px-3 py-2.5">
                    <div className="h-5 w-5 animate-pulse bg-input" />
                    <div className="h-3 w-16 animate-pulse bg-input" />
                    <div className="ml-auto h-3 w-4 animate-pulse bg-input" />
                  </div>
