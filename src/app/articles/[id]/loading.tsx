export default function Loading() {
  return (
    <div className="mx-auto max-w-4xl space-y-8">
      ...
      <div className="space-y-2 border-l-4 border-border-t pl-4">
        <div className="h-5 w-full animate-pulse rounded bg-input" />
        <div className="h-5 w-2/3 animate-pulse rounded bg-input" />
      </div>

      {/* Article body */}
      <div className="space-y-3 rounded-2xl bg-card border border-border-t p-5 sm:p-8">
        {Array.from({ length: 9 }).map((_, i) => (
          <div key={i} className={`h-4 animate-pulse rounded bg-input ${i % 4 === 3 ? "w-1/2" : "w-full"}`} />
        ))}
      </div>
    </div>
  );
